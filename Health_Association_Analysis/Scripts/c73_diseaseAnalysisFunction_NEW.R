...
{
  # making sure that the all the diseases form uniqueDiseases are present in diseaseSampleList
...
associationOutput= function(intermediateOutput, uniqueDiseases, outputFolder, speciesGroupingVariable, speciesGroupingColour, rowDendogramed, colDendogramed)
{
  ...
  write.csv(as.data.frame(t(meanBasedOutput)), paste0(outputFolder,"\\mean_based_output.csv"), row.names = T, quote = F)
  
  print("getting the heatmap carpet")
  pdf("G:\\My Drive\\HACK paper\\CellReports_revision\\disease_analysis_overall_output\\dummy_heatmap.pdf", height = 10, width = 20)
  ...
  # associationCount$healthAssociationScore_division= (associationCount$totalNegativeSignificant+1)/(associationCount$totalPositiveSignificant+1)
  # associationCount$Disease_direction= associationCount$totalNegativeSignificant- associationCount$totalPositiveSignificant
  ...
  # associationCount$healthAssociationScore_division= rank_scale(associationCount$healthAssociationScore_division)
  # associationCount$healthAssociationScore_subtraction= rank_scale(associationCount$Disease_direction)
  ...
  ggsave("linePlot.pdf", plot, path = outputFolder, width = 20, height= 4)
  dev.off()
  return() 
}
...
diseaseAnalysisIterationPipeline= function(iteration, allDiseases, speciesGroupingVariable, speciesGroupingColour, controlSampleList, diseaseSampleList, MainOutputFolder, AllCombinedSpProfile, species_to_check, speciesGroupList= list())
{
  ...
  # stroing the results of all iterations in the list.
	...
	for(folder in allIterationFolders)
	{
	  ...
	  # iterationOutputDf_direction[species_to_check,folder]= heatmapInfo[species_to_check,"Disease_direction"]
	  # iterationOutputDf_divisionScore[species_to_check,folder]= heatmapInfo[species_to_check,"healthAssociationScore_division"]
	  # iterationOutputDf_subtractionScore[species_to_check,folder]= heatmapInfo[species_to_check,"healthAssociationScore_subtraction"]
	...
	# outputList[["iterationOutputDf_direction"]]= iterationOutputDf_direction
	# outputList[["iterationOutputDf_divisionScore"]]= iterationOutputDf_divisionScore
	# outputList[["iterationOutputDf_subtractionScore"]]= iterationOutputDf_subtractionScore
