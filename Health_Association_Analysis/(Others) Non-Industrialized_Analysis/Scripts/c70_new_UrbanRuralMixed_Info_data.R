## Replicating c70_ValidationDiseaseSampleInfo.R for UrbanRuralMixed cohort

# Omprakash
# 25-09-2024

# the code will get the information of all the diseases and their control and disease samples for UrbanRuralMixed cohort


## Load the UrbanRuralMixed_metadata 
...
UrbanRuralMixedCohortInfo= as.data.frame(matrix(nrow= rowCount, ncol= 4))
colnames(UrbanRuralMixedCohortInfo)= c("disease", "study", "controlcount", "diseaseCount")
...
for(disease in uniqueDiseases)
{
  ...
  for(study in uniqueStudies)
  {
    ...
    controlMeta= UrbanRuralMixed_metadata %>% 
    ...
    UrbanRuralMixedCohortInfo[cnt,]= currRow
    cnt= cnt+1
  }
  
}

UrbanRuralMixedCohortInfo$controlcount= as.numeric(UrbanRuralMixedCohortInfo$controlcount)
UrbanRuralMixedCohortInfo$diseaseCount= as.numeric(UrbanRuralMixedCohortInfo$diseaseCount)
...
UrbanRuralMixedCohortInfo$`60_40`= NA
UrbanRuralMixedCohortInfo$`70_30`= NA
UrbanRuralMixedCohortInfo$`80_20`= NA

for(index in 1:nrow(UrbanRuralMixedCohortInfo))
{
  total= UrbanRuralMixedCohortInfo[index,"controlcount"]  + UrbanRuralMixedCohortInfo[index, "diseaseCount"]
  value= UrbanRuralMixedCohortInfo[index,"controlcount"]/total
  
  if(value<0.4 | value>0.6)
  {
    UrbanRuralMixedCohortInfo[index,"60_40"]=0
  }
  else
  {
    UrbanRuralMixedCohortInfo[index,"60_40"]=1
  }
  
  
  if(value<0.3 | value>0.7)
  {
    UrbanRuralMixedCohortInfo[index,"70_30"]=0
  }
  
  else
  {
    UrbanRuralMixedCohortInfo[index,"70_30"]=1
  }
  
  if(value<0.2 | value>0.8)
  {
    UrbanRuralMixedCohortInfo[index,"80_20"]=0
  }
  
  else
  {
    UrbanRuralMixedCohortInfo[index,"80_20"]=1
  }
  
  
}

write_xlsx(UrbanRuralMixedCohortInfo, "C:/Users/ompra/Downloads/Cell_reports_revision/disease_analysis/new_files/UrbanRuralMixed/c70_new_UrbanRuralMixed_Info.xlsx")


save.image("C:/Users/ompra/Downloads/Cell_reports_revision/disease_analysis/new_files/UrbanRuralMixed/c70_new_UrbanRuralMixed_Info_data.RData")
