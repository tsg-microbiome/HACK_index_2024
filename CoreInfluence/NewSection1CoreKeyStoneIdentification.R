...
CoreKeyStoneDf <- apply(r2Df,2,function(x)(ifelse(x>=0.70,1,0))) * apply(prevalentDf,2,function(x)(ifelse(x>=0.70,1,0)))
...
mat_CoreKeyStone_Detection <- data.frame(A1=apply(mat_CoreKeyStone[,names(which(CohortLifeStyle=="IndustrializedUrban"))],1,function(x)(length(x[x==0]))),A3=apply(mat_CoreKeyStone[,names(which(CohortLifeStyle=="IndustrializedUrban"))],1,function(x)(length(x[x!=0]))),A3=apply(mat_CoreKeyStone[,names(which(CohortLifeStyle!="IndustrializedUrban"))],1,function(x)(length(x[x==0]))),A4=apply(mat_CoreKeyStone[,names(which(CohortLifeStyle!="IndustrializedUrban"))],1,function(x)(length(x[x!=0]))))
...
#heatmap.2(study_aggregated_mat_core[rownames(mat_CoreKeyStone),],density="none",trace="none",Colv=FALSE,Rowv=FALSE,col=brewer.pal(8,"Greens"),lhei=c(0.1,5),margins=c(5,55),srtRow=0.5,sepcolor="black",sepwidth=c(0.01,0.01),colsep=0:ncol(study_aggregated_mat_core),rowsep=0:nrow(study_aggregated_mat_core))
...
#heatmap.2(t(dfAllAbundanceAssociationsR),density="none",trace="none",Rowv=FALSE,Colv=FALSE,col=brewer.pal(8,"PuOr"),margins=c(30,5),lhei=c(0.1,5),lwid=c(0.1,5),cellnote=apply(t(dfAllAbundanceAssociationsP),2,function(x)(ifelse(x<=0.05,"*",""))),notecol="black",notecex=2)
