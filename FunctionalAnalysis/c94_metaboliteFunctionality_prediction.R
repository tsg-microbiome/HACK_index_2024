...
# Input Files:
# 1) combined_metabolite_map.RData
# 2) MetaboliteMap_Output.RData
# 3) SpeciesScores.RData

# Output Files:
# 1) c94_logisticFeatures_volcanoPlot.pdf
# 2) c94_MetabolitePrediction_plot.pdf
# 3) c94_metabolite_selectedFeatures.RData
...
load("G:/My Drive/IIITD/project/control_runs/input_files/FunctionalProfiling/combined_metabolite_map.RData")
load("G:/My Drive/IIITD/project/control_runs/output_files/c102_funtionalProfileOutput/MetaboliteMap_Output.RData")
load("G:/My Drive/IIITD/project/control_runs/input_files/FunctionalProfiling/SpeciesScores.RData")

# metabolite map doesn't contain information about all the species for which other functional information is present, therefore taking the common species only.
# metaboliteMap_functionalProfile= df_select_metabolite_map[intersect(rownames(df_select_metabolite_map),rownames(df_select_features)),]
...
# pdf(file = "G:\\My Drive\\IIITD\\project\\control_runs\\output_files\\images\\c94_logisticFeatures_volcanoPlot.pdf", width= 15)
ggplot(logisticModel_Df, aes(x=Estimate,
  ...
  geom_label_repel(data= subset(logisticModel_Df, p_value<=0.1),aes(label= labels),
                   size= 5.5, segment.size= 0.2, force= 8.2, label.size = NA)+
  geom_hline(yintercept = 1, linetype = "dotted", size = 1)+
  geom_hline(yintercept = 1.301, linetype = "dotted", size = 1)+
  geom_vline(xintercept = 0, size = 1)
  
ggsave("G:\\My Drive\\IIITD\\project\\control_runs\\output_files\\images\\c94_logisticFeatures_volcanoPlot.pdf", width = 7.15, height = 2.41)
...
# pearson correlation
cor(rf_MetaboliteMap$y, rf_MetaboliteMap$predicted) # 0.49
...
cor(rf_logistic_selectedFeatures$y, rf_logistic_selectedFeatures$predicted, method= "spearman") # 0.54
...
pdf(file = "G:\\My Drive\\IIITD\\project\\control_runs\\output_files\\images\\c94_MetabolitePrediction_plot.pdf")
...
save(logisticModel_Df, filtered_logisticModel_df
     , file= "G:\\My Drive\\IIITD\\project\\control_runs\\output_files\\c94_metabolite_selectedFeatures.RData")
