...
# 18-12-2023
# code name- c87_hackAssociated_functinonal_Ids.R

# Input Files:
# 1) c93_logisticRegression_FeatureSelection.RData
# 2) FunctionalClassificationAnalysis_stage2.RData

# Output Files:
# 1) c87_HackFinal_Features.pdf
# 2) c87_HackFinal_Features_dotPlot.pdf
# 3) c87_hackAssociated_functinonal_Ids.RData

# the code will find out the only hack associated genes that are significantly present in higher abundance
# in hack species than in non hack species.


library(dplyr)

load("G:/My Drive/IIITD/project/control_runs/output_files/c93_logisticRegression_FeatureSelection.RData")
load("G:/My Drive/IIITD/project/control_runs/input_files/FunctionalProfiling/FunctionalClassificationAnalysis_stage2.RData")
...
hackGenes= function(functionalProfile)
{
  ...
  # dividing the data frame based on hack and non hack species, the threshold for hackScore is 0.7.
  hackDf<<- as.data.frame(t(functionalProfile[which(functionalProfile$HACKScore>=0.7),]))
  nonHackDf<<- as.data.frame(t(functionalProfile[which(functionalProfile$HACKScore<0.7),]))
...
# there are two features GH3 with same name so adding them into one feature
index= which(grepl("GH3$",colnames(df_select_features)))
colnames(df_select_features)[index]

df_select_features[,index[1]]= df_select_features[,index[1]]+ df_select_features[,index[2]]

df_select_features= df_select_features[,-index[2]]

# running the above function for all the above funcitons.
...
# loading the logistic regression based features generated in code c92.
# Intersect between the wicox based features and logistic regression based features are taken as hack associated features.
# Union of them will be taken to generate the protein database.

# intersect_hackAssociatedGenes_Df= df_select_features[,intersect(rownames(check), rownames(logisticModel_Df))]
union_hackAssociatedGenes_Df= df_select_features[,union(rownames(check), rownames(logisticModel_Df))]
...
pdf(file = "G:\\My Drive\\IIITD\\project\\control_runs\\output_files\\images\\c87_wilcoxFeatures.pdf", width = 20, height = 10);
...
pdf(file = "G:\\My Drive\\IIITD\\project\\control_runs\\output_files\\images\\c87_HackFinal_Features.pdf", width = 20, height = 10);
ggplot(union_df_2, aes(x = Features, y = species, fill = factor(presence)))+
  geom_tile()+
  scale_fill_manual(values = color_vector) +
  theme(axis.text.y = element_text(size = 5,color = "black"))+
...
FeatureCountData$rank= c(1:121)


cor(FeatureCountData$FeatureCount, FeatureCountData$HACKScore) # 0.58


pdf(file = "G:\\My Drive\\IIITD\\project\\control_runs\\output_files\\images\\c87_HackFinal_Features_dotPlot.pdf");
...
save(final_HackFunctionalProfile,
     ...
     hackSpecies,
     nonHackSpecies,
     file= "G:\\My Drive\\IIITD\\project\\control_runs\\output_files\\c87_hackAssociated_functinonal_Ids.RData")
