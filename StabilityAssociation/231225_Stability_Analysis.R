...
combined_rel_abund_followup_dist_all_196sp_df <- as.data.frame(rbind(combined_rel_abund_followup_dist_discovery_196sp_df,combined_rel_abund_followup_dist_validation_196sp_df))

combined_df <- combined_rel_abund_followup_dist_all_196sp_df

detected_taxa <- colnames(combined_df)[1:196]
...
median_stability_Aitchison <- apply(iteration_stability_Aitchison,1,function(x)(median(x[!is.na(x)])))
IQR_stability_Aitchison <- apply(iteration_stability_Aitchison,1,function(x)(IQR(x[!is.na(x)])))
stability_aitchison <- data.frame(median=median_stability_Aitchison,IQR=IQR_stability_Aitchison)
...
stability_AitchisonDist <- data.frame(median=apply(iteration_stability_Aitchison,1,function(x)(median(x[!is.na(x)]))),IQR=apply(iteration_stability_Aitchison,1,function(x)(IQR(x[!is.na(x)]))))
...
stability_BrayCurtisDist <- data.frame(median=apply(iteration_stability_BrayCurtis,1,function(x)(median(x[!is.na(x)]))),IQR=apply(iteration_stability_BrayCurtis,1,function(x)(IQR(x[!is.na(x)]))))
...
colnames(stabilityRank) <- c("median_Aitchison","IQR_Aitchison","median_BrayCurtis","IQR_BrayCurtis")
...
load("G:\\My Drive\\Lab\\Projects\\CoreFinder\\FunctionalProfiling\\InfluenceScore.RData")

InfluenceStabilityScore <- data.frame(StabilityScore=stabilityRank$MeanStabilityScore,InfluenceScore=InfluenceScore[rownames(stabilityRank),1],row.names=rownames(stabilityRank))
...
Directionality_FinalCoreKeyStone <- data.frame(Overall_Aitchison=combined_AitchisonDist[FinalCoreKeyStones,"dir"],Aitchison_Adjusted=MetaLM_Aitchison_AbundanceDiff_Adjusted[FinalCoreKeyStones,"dir"],Aitchison_UC=MetaLM_Aitchison_UC[FinalCoreKeyStones,"dir"],Aitchison_CD=MetaLM_Aitchison_CD[FinalCoreKeyStones,"dir"],Aitchison_Controls=MetaLM_Aitchison_Control[FinalCoreKeyStones,"dir"],Aitchison_Fiber=MetaLM_Aitchison_Fiber[FinalCoreKeyStones,"dir"],Aitchison_Others=MetaLM_Aitchison_Others[FinalCoreKeyStones,"dir"],Overall_BrayCurtis=combined_BrayCurtisDist[FinalCoreKeyStones,"dir"],BrayCurtis_Adjusted=MetaLM_BrayCurtis_AbundanceDiff_Adjusted[FinalCoreKeyStones,"dir"],BrayCurtis_UC=MetaLM_BrayCurtis_UC[FinalCoreKeyStones,"dir"],BrayCurtis_CD=MetaLM_BrayCurtis_CD[FinalCoreKeyStones,"dir"],BrayCurtis_Controls=MetaLM_BrayCurtis_Control[FinalCoreKeyStones,"dir"],BrayCurtis_Fiber=MetaLM_BrayCurtis_Fiber[FinalCoreKeyStones,"dir"],BrayCurtis_Others=MetaLM_BrayCurtis_Others[FinalCoreKeyStones,"dir"],row.names=FinalCoreKeyStones)

Directionality_FinalCoreKeyStone <- apply(Directionality_FinalCoreKeyStone,2,function(x)(ifelse(is.na(x),0,x)))
